...
// PREPWORK
var d=this.options,e=this;this.initVal=e.element.val(),d.ajax&&(d.live=!1),
// create the wrapper
...
d.ajax_debounce&&(f=e._debounce(f,d.ajax_debounce)),
...
e._hideChoices(e.wrapper)}}).on("keyup",f),
...
g!=e.element.val()&&null!=e.element.val()&&""!=e.element.val()?(g=e.element.val(),"array"==typeof g?g.each(function(a,b){e._selectChoice(e.wrapper.find("li[data-value='"+b+"']"))}):e._selectChoice(e.wrapper.find("li[data-value='"+g+"']"))):null!=e.element.val()&&""!=e.element.val()||(g=e.element.val(),e.items.removeClass(e.options.class_selected),e.input.text("").attr("data-placeholder",e.options.placeholder)),
...
// set cache to the original value
var g=this.element.val(),h=function(){
// if we're out of date
g!=e.element.val()&&null!=e.element.val()&&""!=e.element.val()?(g=e.element.val(),"array"==typeof g?g.each(function(a,b){e._selectChoice(e.wrapper.find("li[data-value='"+b+"']"))}):e._selectChoice(e.wrapper.find("li[data-value='"+g+"']"))):null!=e.element.val()&&""!=e.element.val()||(g=e.element.val(),e.items.removeClass(e.options.class_selected),e.input.text("").attr("data-placeholder",e.options.placeholder)),
// let's also check for disabled
e.element.prop("disabled")?e.wrapper.addClass(d.class_disabled):e.wrapper.removeClass(d.class_disabled)};e.element.on("change",h)}
// after init callback
d.afterinit()},
// PRIVATE METHODS
...
_debounce:function(a,b,c){var d,e,f,g,h,i=function(){var j=(new Date).getTime()-g;b>j&&j>=0?d=setTimeout(i,b-j):(d=null,c||(h=a.apply(f,e),d||(f=e=null)))};return function(){f=this,e=arguments,g=(new Date).getTime();var j=c&&!d;return d||(d=setTimeout(i,b)),j&&(h=a.apply(f,e),f=e=null),h}},
...
a.ajax({url:c.ajax,data:{q:this.input.text()},dataType:"json",type:"GET",success:function(e){if(
// we got a response
c.debug&&console.log("Minimalect received ",e," for query '"+d.input.text()+"' in ",d.element),e.length){
// if we have results
var f="";f+="<option></option>",a.each(e,function(a,b){
